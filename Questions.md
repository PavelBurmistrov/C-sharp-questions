...
### Что такое стэк? Как он работает?
### Что такое куча? Как она работает?
### Сколько может быть стэков и куч в выполняемой программе? Почему именно так?
### Примитивные + значимые типы, примеры, где они хранятся
 
 Примитивные типы данных - те, о которых знает компилятор C# (Int32, Int64, UInt32, Uint64, char, Byte, SByte, Single, Double, Decimal)
...
### Как работает params для параметров методов?
 ...
 Массив параметров, можно передавать переменное количество параметров. Если использовать тип object, то можно передавать значения разных типов. Пример: void Method(char c, params int[] numbers){}
...
### Посмотреть что такое Singleton паттерн и как его реализовать
### Посмотреть оператор lock синхронизации доступа в .NET, понять как он работает, связать его с блоком индекса синхронизации
